...
def main():
	predicate1 = re.compile(r'sample(.*).txt')
...
def plotn(files, interval="-10:3600", yrange="-10:110"):
	# plot normal
	p = subprocess.Popen(['gnuplot', '-'], stdin=subprocess.PIPE)
	cmds = []
	cmds.append("set term pngcairo")
	cmds.append("set out \"distancephyINT.png\" " )
	cmds.append("set title \"Brightness over different distances, with ambient light interference\"")
	...
	for samplefile in files:
		if '100' in samplefile:
			plotstr += ("\"x" + samplefile + "\" w linespoints title \"" + samplefile[7:].rstrip('.txt') + " cm\" pt 12,")
		else:
			plotstr += ("\"x" + samplefile + "\" w linespoints title \"" + samplefile[6:].rstrip('.txt') + " cm\" pt 12,")
		i +=1 
